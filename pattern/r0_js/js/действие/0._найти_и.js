...
// найти простое число
// число не простое если делится на 2
  for(let i=2;i<num;i++){if(num%i==0){return false;}}
// Простое число делится без остатка только на 1 и на себя.
// Не_простое_число делится без остатка на некоторые числа перед ним, на себя, на 1.
let i=2, rPrime='';
nextPrime:
for(i;i<n;i++){
  for(let j=2; j<i; j++){
...
// найти делители Не_простого_числа integer
  for(let i=2; i<integer; i++)
    if(integer%i===0)rArr.push(i);//сложим в rArr
  if(rArr.length==0)// находим простое\prime число
